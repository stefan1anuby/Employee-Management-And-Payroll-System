    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .authorizeHttpRequests(authz -> authz
                        .requestMatchers("/login/oauth2/**", "/auth-success", "/api/user-info").permitAll() // Allow login paths
                        .anyRequest().authenticated() // Require authentication for other paths
                )
                .oauth2Login(oauth2 -> oauth2
                        .successHandler(customAuthenticationSuccessHandler) // Use custom success handler for JWT generation
                );
